    ...
    productCarousel.innerHTML = productsToShow.map((product) => {
        return `<div class="product-card">
                    ...
                    <div class="product-prices-container">
                        <p class ="product-price-discounted">$${product.price * 0.5}</p>
                        <s class ="product-price"> $${product.price}</s>
                     </div>
                    <p>${product.rating.rate} (${product.rating.count})</p>
    
                    <div class="product-card-icons">
                        <img onclick="addToWishlist(${product.id})" src="images/wishlist-icon.svg" class="wishlist-icon" />
                        <img onclick="addToCart(${product.id})" src="images/cart-icon.svg"  class="cart-icon" />
                     </div>
...
function deleteFromWishlist (deletedProductId) {
    const wishlistProducts = JSON.parse(localStorage.getItem("wishlistProducts")) || [];
    const filteredProducts = wishlistProducts.filter(product => product.id !== deletedProductId);
...
function deleteFromCart (deletedProductId) {
    const cartProducts = JSON.parse(localStorage.getItem("cartProducts")) || [];
    const filteredProducts = cartProducts.filter(product => product.id !== deletedProductId);
...
function showAllProducts() {
    productCarousel.innerHTML = products.map((product) => {
        return `<div class="product-card">
                    <img class ="product-card-img" src ="${product.image}" alt = "${product.title}" />
                    <button class="add-to-cart-btn" id="addToCartBtn">Add To Cart</button>
                    <p class = "discount-rate">-50%</p>
